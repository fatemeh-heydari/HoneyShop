...
def signup_user(request):
    ...
    if (request.method == "POST"):
        ...
        if (form.is_valid()):
            ...
            messages.error(request, ('مشکلی در ثبت نام شما وجود دارد لطفا اطلاعات را با دقت وارد کنید'))
...
def update_user(request):
    if request.user.is_authenticated:
        ...
        # خط بالا برای این است که وقتی کاربر به صفحه ویرایش میرود اطلاعات ثبت شده ی خود را مشاهده کند و در صفحه باشد (برای گرفتن اطلاعات کاربر)
    ...
    if request.user.is_authenticated:
        ...
        if request.method =='POST' :
            ...
            if form.is_valid():
                ...
                # forms.eror  یک دیکشنری است که مقادیر(ولیو) آن توضیحات خطاهای موجود در برنامه است که میتواینیم آن را به کاربر  نمایش دهیم
...
def update_info(request):
    if request.user.is_authenticated:
        ...
        shipping_user, created_shipping = ShippingAddress.objects.get_or_create(user=request.user)
...
# نشان دادن سفارشات ثبت شده ی کاربر به خود کاربر در یک صفحه ی جداگانه
def user_orders(request):
    ...
    # برای نمایش جزییات یک سفارش از طریق لیست سفارشات
